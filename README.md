...
- Multi-broker adapters for Zerodha, Upstox, and Binance spot markets with unified `.env` configuration.
- Incremental parquet pipeline (`--mode update`, `data_fetcher.py`) to append data instead of rebuilding pools.
- Edit-in-place data tools (gap calculator, pool inspector, token manager) for long-lived repositories.
- Guardrail validators (config, signal parity, precision) with targeted pytest suites.
- Lean analysis toolkit: curated `analysis/` modules and YAML templates, legacy option artefacts archived.
- Repository cleanup: options stack, AI prompt scaffolding, and large CSV pools removed from the OSS footprint.
...
The `update` mode shares logic with `src/core/etl/data_fetcher.py` and supports:
...
1. Inspect existing pool: `python src/core/etl/pool_inspector.py --pool-path data/pools/2024-01-01_to_2024-06-30`
2. Calculate gaps before fetching: `python src/core/etl/gap_calculator.py --pool-path ...`
3. Fetch or update using `data_fetcher.py` or `--mode update` from the unified runner.

Small sample metadata lives in `data/indian_equities_master.csv` for quick reference. Large historical CSVs/parquet files are intentionally omitted from the OSS repository.
...
See `docs/strategylab_v2_phase0_audit.md` for full decision history.
