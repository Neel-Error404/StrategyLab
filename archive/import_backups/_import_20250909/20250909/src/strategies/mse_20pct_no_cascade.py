# strategies/mse_20pct_no_cascade.py
# MSE Strategy: 20% Exit Threshold WITHOUT CASCADE PREVENTION
...
class MSEStrategy(StrategyBase):
    ...
    def __init__(self, name="MSE_20pct_NoCascade", parameters=None):
    ...
    def prepare_strategy_data(
        ...
        logging.info(f"MSE Strategy 20% NO Cascade: Starting for {ticker}, date={pull_date}, total rows={len(base_1m)}")
        ...
        logging.info(f"MSE Strategy 20% NO Cascade: After warm-up, final rows={len(final_df)} for {ticker} on '{pull_date}'")
        ...
        logging.info(f"MSE Strategy 20% NO Cascade: Completed for {ticker} on {pull_date}")
