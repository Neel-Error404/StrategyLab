# strategies/mse_80pct_with_cascade.py
# MSE Strategy: 80% Exit Threshold WITH CASCADE PREVENTION
...
class MSEStrategy(StrategyBase):
    ...
    def __init__(self, name="MSE_80pct_WithCascade", parameters=None):
    ...
    def prepare_strategy_data(
        ...
        logging.info(f"MSE Strategy 80% WITH Cascade: Starting for {ticker}, date={pull_date}, total rows={len(base_1m)}")
        ...
        logging.info(f"MSE Strategy 80% WITH Cascade: After warm-up, final rows={len(final_df)} for {ticker} on '{pull_date}'")
        ...
        logging.info(f"MSE Strategy 80% WITH Cascade: Completed for {ticker} on {pull_date}")
