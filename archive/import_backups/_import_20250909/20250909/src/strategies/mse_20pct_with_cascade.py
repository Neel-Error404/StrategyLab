# strategies/mse_20pct_with_cascade.py
# MSE Strategy: 20% Exit Threshold WITH CASCADE PREVENTION
...
class MSEStrategy(StrategyBase):
    ...
    def __init__(self, name="MSE_20pct_WithCascade", parameters=None):
        super().__init__(name, parameters or {})
        # CASCADE PREVENTION: ENABLED
        self.enable_cascade_prevention = True
        # EXIT THRESHOLD: 20%
    ...
    def prepare_strategy_data(
        ...
        logging.info(f"MSE Strategy 20% WITH Cascade: Starting for {ticker}, date={pull_date}, total rows={len(base_1m)}")
        ...
        logging.info(f"MSE Strategy 20% WITH Cascade: After warm-up, final rows={len(final_df)} for {ticker} on '{pull_date}'")
            ...
            if in_buy_trade and not buy_exit_pending:
                ...
                if prev_macd_hist is not None:
                    ...
                    if buy_peak_initialized and prev_macd_hist < self.exit_threshold * buy_max_hist:
                        ...
                        logging.info(f"Buy exit condition detected using PREVIOUS bar data - will exit at {row['timestamp']} OPEN. MACD Hist: {prev_macd_hist}")
            ...
            if in_sell_trade and not sell_exit_pending:
                ...
                if prev_macd_hist is not None:
                    ...
                    if sell_peak_initialized and prev_macd_hist > self.exit_threshold * sell_min_hist:
                        ...
                        logging.info(f"Sell exit condition detected using PREVIOUS bar data - will exit at {row['timestamp']} OPEN. MACD Hist: {prev_macd_hist}")

        logging.info(f"MSE Strategy 20% WITH Cascade: Completed for {ticker} on {pull_date}")
