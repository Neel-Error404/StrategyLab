...
class MSEStrategy(StrategyBase):
    ...
    def prepare_strategy_data(
        ...
        logging.info(f"MSE Strategy 80% NO Cascade: Starting for {ticker}, date={pull_date}, total rows={len(base_1m)}")
        ...
        logging.info(f"MSE Strategy 80% NO Cascade: After warm-up, final rows={len(final_df)} for {ticker} on '{pull_date}'")

        # 5) Define buy/sell logic aligned to live execution (decide at close → fill next open)
        # Initialize all signal columns and price-hint columns
        final_df['entry_signal_buy'] = False
        final_df['entry_signal_sell'] = False
        ...
        # Generate raw technical signals using CURRENT bar indicators (decision at close)
        raw_buy_signal = (
        ...
        # Apply CASCADE PREVENTION filter if enabled (disabled for this variant)
        if self.enable_cascade_prevention:
            filtered_buy, filtered_sell = self._apply_cascade_prevention(
            ...
            filtered_buy = raw_buy_signal
            filtered_sell = raw_sell_signal
        ...
        for idx, row in final_df.iterrows():
            ...
            if buy_entry_pending and not in_buy_trade:
                # Mark current row as actual entry and request OPEN price fill
            ...
            # Detect new entry signals using PREVIOUS bar decision → schedule fill on CURRENT bar's OPEN
            if not in_buy_trade and not buy_entry_pending and filtered_buy.iloc[idx-1]:
                buy_entry_pending = True
                logging.info(f"Buy entry detected at {final_df.iloc[idx-1]['timestamp']} CLOSE → fill CURRENT OPEN")

            if not in_sell_trade and not sell_entry_pending and filtered_sell.iloc[idx-1]:
                sell_entry_pending = True
                logging.info(f"Sell entry detected at {final_df.iloc[idx-1]['timestamp']} CLOSE → fill CURRENT OPEN")

            # Update peak/valley tracking and check exit conditions using PREVIOUS bar data
            # Avoids look-ahead bias; original backtest behavior
            if in_buy_trade and not buy_exit_pending:
                ...
                if prev_macd_hist is not None:
                    ...
                    # Check exit condition for Buy using PREVIOUS bar - CONFIGURABLE THRESHOLD
                    if buy_peak_initialized and prev_macd_hist < self.exit_threshold * buy_max_hist:
                        buy_exit_pending = True
                        logging.info(
                            f"Buy exit condition detected using PREVIOUS bar data - will exit at {row['timestamp']} OPEN. "
                            f"MACD Hist: {prev_macd_hist}, Peak: {buy_max_hist}, Threshold: {self.exit_threshold}"
                        )
            ...
            if in_sell_trade and not sell_exit_pending:
                ...
                if prev_macd_hist is not None:
                    # Initialize valley tracking on first iteration after entry
                    ...
                    # Check exit condition for Sell using PREVIOUS bar - CONFIGURABLE THRESHOLD
                    if sell_peak_initialized and prev_macd_hist > self.exit_threshold * sell_min_hist:
                        sell_exit_pending = True
                        logging.info(
                            f"Sell exit condition detected using PREVIOUS bar data - will exit at {row['timestamp']} OPEN. "
                            f"MACD Hist: {prev_macd_hist}, Valley: {sell_min_hist}, Threshold: {self.exit_threshold}"
                        )

        logging.info(f"MSE Strategy 80% NO Cascade: Completed for {ticker} on {pull_date}")
