...
class MSEStrategy(StrategyBase):
    """
    MSE (Mean Squared Error) strategy implementation with CASCADE PREVENTION.
    
    NEW FEATURE: Direction Alternation Rule
    - Prevents consecutive same-direction trades (Buy after Buy, Sell after Sell)
    - Requires opposite direction trade between same-direction entries
    - Based on cascading effect analysis showing $3M annual profit loss from re-entries
    """
   
    def __init__(self, name="MSE", parameters=None):
        super().__init__(name, parameters or {})
        # CASCADE PREVENTION: Enabled to prevent same-direction trades on same day
        self.enable_cascade_prevention = True  # Feature flag
    ...
    last_processed_timestamp: Optional[pd.Timestamp] = None
        ...
        # 5) Define buy/sell logic with CASCADE PREVENTION
    ...
    def _apply_cascade_prevention(self, df: pd.DataFrame, raw_buy_signal: pd.Series, 
        ...
        RULE: Second trade of the day must be opposite direction to first trade.
        Third trade must be opposite direction to second trade, etc.
        
        This prevents the devastating re-entry cascade pattern identified in analysis.
        
        Args:
            df: DataFrame with timestamp and signal data
            raw_buy_signal: Raw buy signals before filtering
            raw_sell_signal: Raw sell signals before filtering  
            ticker: Stock ticker symbol
            pull_date: Trading date
            
        Returns:
            Tuple of (filtered_buy_signal, filtered_sell_signal)
        """
        ...
        logging.info(f"  Daily sequences: {daily_trade_log}")
