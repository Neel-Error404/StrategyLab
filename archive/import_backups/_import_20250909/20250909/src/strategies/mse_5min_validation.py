...
class MSEStrategy(StrategyBase):
    ...
    def prepare_strategy_data(
        ...
        logging.info(f"MSE 5-Minute Validation - Matches Live Timing: Starting for {ticker}, date={pull_date}, total rows={len(base_1m)}")
        ...
        logging.info(f"MSE 5-Minute Validation - Matches Live Timing: After warm-up, final rows={len(final_df)} for {ticker} on '{pull_date}'")
        ...
        # Generate raw technical signals using CURRENT completed candle (like live) indicators to match live system timing (5-minute intervals only)
        raw_buy_signal = (
        ...
        # CRITICAL: Only check signals every 5 minutes to match live system
        for idx, row in final_df.iterrows():
            # Only process rows that are 5-minute boundaries (like live system)
            current_minute = row['timestamp'].minute
            if current_minute % 5 != 0:
                continue  # Skip non-5-minute boundaries
            ...
            # Update peak/valley tracking and check exit conditions using CURRENT completed candle (like live) data
            if in_buy_trade and not buy_exit_pending:
                # Use CURRENT completed candle (like live) MACD Histogram to match live system timing (5-minute intervals only)
                ...
                if prev_macd_hist is not None:
                    ...
                    if not buy_peak_initialized:
                        ...
                        logging.info(f"Buy peak tracking initialized with CURRENT completed candle (like live) MACD: {buy_max_hist}")
                    
                    # Update maximum MACD Histogram since entry using CURRENT completed candle (like live)
                    if prev_macd_hist > buy_max_hist:
                        buy_max_hist = prev_macd_hist

                    # Check exit condition for Buy using CURRENT completed candle (like live) - CONFIGURABLE THRESHOLD
                    if buy_peak_initialized and prev_macd_hist < self.exit_threshold * buy_max_hist:
                        buy_exit_pending = True
                        logging.info(f"Buy exit condition detected using CURRENT completed candle (like live) data - will exit at {row['timestamp']} OPEN. MACD Hist: {prev_macd_hist}")
            ...
            if in_sell_trade and not sell_exit_pending:
                ...
                if prev_macd_hist is not None:
                    ...
                    if not sell_peak_initialized:
                        ...
                        logging.info(f"Sell peak tracking initialized with CURRENT completed candle (like live) MACD: {sell_min_hist}")
                    
                    # Update minimum MACD Histogram since entry using CURRENT completed candle (like live)
                    if prev_macd_hist < sell_min_hist:
                        sell_min_hist = prev_macd_hist

                    # Check exit condition for Sell using CURRENT completed candle (like live) - CONFIGURABLE THRESHOLD
                    if sell_peak_initialized and prev_macd_hist > self.exit_threshold * sell_min_hist:
                        sell_exit_pending = True
                        logging.info(f"Sell exit condition detected using CURRENT completed candle (like live) data - will exit at {row['timestamp']} OPEN. MACD Hist: {prev_macd_hist}")

        logging.info(f"MSE 5-Minute Validation - Matches Live Timing: Completed for {ticker} on {pull_date}")
