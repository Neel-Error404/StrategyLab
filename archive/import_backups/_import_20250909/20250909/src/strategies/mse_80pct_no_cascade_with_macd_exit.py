# strategies/mse_80pct_no_cascade_with_macd_exit.py
# MSE Strategy: 80% Exit Threshold + MACD Crossover Exits WITHOUT CASCADE PREVENTION
...
class MSEStrategy(StrategyBase):
    ...
    1. MACD histogram drops below 80% of peak (for BUY) / rises above 80% of valley (for SELL)
    ...
    def __init__(self, name="MSE_80pct_NoCascade_WithMACDExit", parameters=None):
    ...
    def prepare_strategy_data(
        ...
        logging.info(f"MSE Strategy 80% NO Cascade + MACD Exit: Starting for {ticker}, date={pull_date}, total rows={len(base_1m)}")
        ...
        logging.info(f"MSE Strategy 80% NO Cascade + MACD Exit: After warm-up, final rows={len(final_df)} for {ticker} on '{pull_date}'")
        ...
        logging.info(f"MSE Strategy 80% NO Cascade + MACD Exit: Completed for {ticker} on {pull_date}")
