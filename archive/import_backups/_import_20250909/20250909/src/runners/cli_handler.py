...
Examples:
  ...
  # Interactive fetch mode (prompts for all parameters)
...
def load_config_from_args(args) -> BacktestConfig:
    ...
    # Handle skip flags
    if hasattr(args, 'skip_visualization') and args.skip_visualization:
    ...
    # Handle max workers with validation
    if hasattr(args, 'max_workers') and args.max_workers:
        ...
        if max_workers > recommended_max:
            print(f"⚠️  WARNING: Requested {max_workers} workers exceeds recommended maximum of {recommended_max}")
            print(f"   System has {cpu_cores} CPU cores. High worker counts may cause performance issues.")
            response = input(f"   Continue with {max_workers} workers? [y/N]: ").strip().lower()
            if response not in ['y', 'yes']:
                print(f"   Using recommended maximum of {recommended_max} workers instead.")
                max_workers = recommended_max
        elif max_workers < 1:
            print(f"❌ ERROR: max-workers must be at least 1, got {max_workers}")
            raise ValueError(f"Invalid max_workers value: {max_workers}")
        
        config.execution.max_workers = max_workers
        print(f"✅ Using {max_workers} parallel workers (system has {cpu_cores} CPU cores)")
...
class CLIHandler:
    ...
    def validate_arguments(self, args) -> bool:
        ...
        """
        # Fetch mode always uses interactive interface - ignores CLI parameters
        if args.mode == 'fetch':
            if args.dates or args.date_ranges or args.tickers:
                print("Note: Fetch mode uses interactive interface. CLI parameters will be ignored.")
            return True
