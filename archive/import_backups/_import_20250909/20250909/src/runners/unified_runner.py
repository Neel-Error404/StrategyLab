...
class UnifiedBacktesterRunner:
    ...
    def __init__(self, config: BacktestConfig):
        ...
        # Skip heavy initialization for fetch and validate modes
        mode = getattr(self.config, 'mode', 'backtest')
        if mode in ['fetch', 'validate']:
            # Lightweight initialization for data-only operations
            self.workflow_manager = None
            self.logger.info(f"Initialized {mode} mode (lightweight)")
            return
        
        # Full initialization for backtest/analyze/visualize modes
    ...
    def run(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if mode == 'fetch':
                # Data fetching mode uses its own interactive interface
                self.logger.info("🔄 Launching interactive data fetcher")
                from src.core.etl.data_fetcher import main as interactive_data_fetch
                
                # The fetch system handles its own parameters and user interaction
                try:
                    interactive_data_fetch()
                    results = {'status': 'success', 'mode': 'interactive_fetch'}
                except Exception as e:
                    self.logger.error(f"Data fetch failed: {e}")
                    results = {'status': 'error', 'error': str(e)}
            
            elif mode == 'validate':
                # Validation only - doesn't need task generation
                date_ranges = self.config.strategy.date_ranges if hasattr(self.config.strategy, 'date_ranges') else []
                ...
                # All other modes need task generation (backtest, analyze, visualize)
                # Generate tasks from configuration
                ...
                if mode == 'backtest':
                    ...
                    results = self.workflow_manager.execute_full_workflow(
                        ...
                        skip_visualization=getattr(self.config, 'skip_visualization', False)
                    )
                elif mode == 'analyze':
                    # Analysis workflow
    ...
    def run_backtest(self, dates: List[str], tickers: List[str], strategies: List[str], 
                     optimization_params: Optional[Dict] = None, use_parallel: Optional[bool] = None,
        ...
        # Validate data (skip if requested)
        if not getattr(self.config, 'skip_validation', False):
            ...
            self.logger.info("⏭️  Skipping data validation as requested")
        ...
        # Use CLI config if use_parallel not explicitly provided
        ...
        if mode == 'backtest':
            ...
            return self.workflow_manager.execute_full_workflow(
                ...
                skip_visualization=skip_visualization or getattr(self.config, 'skip_visualization', False)
            )
        elif mode == 'analyze':
            # Analysis workflow
