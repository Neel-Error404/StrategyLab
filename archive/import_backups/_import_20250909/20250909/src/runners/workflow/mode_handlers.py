...
class FetchModeHandler(ModeHandler):
    ...
    def execute(self, dates: List[str], tickers: List[str], 
        ...
        max_workers = getattr(self.config.execution, 'max_workers', None) if hasattr(self.config, 'execution') else None
        
        if use_parallel:
            self.logger.info(f"Parallel processing enabled with {max_workers or 'default'} workers")
