    ...
    for idx, row in df.iterrows():
        ...
        if not in_buy_trade and not in_sell_trade and row.get("entry_signal_buy"):
            ...
            trade = {
                ...
                # Allow strategies to request open-price fills when aligning to live
                "Entry Price": row["open"] if row.get("use_open_for_entry", False) and "open" in row else row["close"],
        ...
        if in_buy_trade:
            ...
            if row.get("exit_signal_buy"):
                ...
                # Honor open-price exit when requested by strategy
                trade["Exit Price"] = row["open"] if row.get("use_open_for_exit", False) and "open" in row else row["close"]
