...
def detect_pool_layout(pool_path: Path) -> str:
    ...
    if parquet_files:
        # Has parquet files directly → could be either layout
        ...
        if any(char.isdigit() for char in file_stem) or file_stem in ['minute', 'day', 'week', 'month', 'hour']:
            # Likely a timeframe → directory is ticker-first
            return 'ticker-first'
        elif file_stem.isupper() or len(file_stem) > 8:
            # Likely a ticker name → directory is timeframe-first
            return 'timeframe-first'
        else:
            # Default to ticker-first as it's more common
            return 'ticker-first'
    else:
        # No parquet files directly → must be ticker-first with nested structure
...
def inspect_pool(pool_path: str, validate: bool = True) -> PoolMetadata:
    ...
    """
    logger.info(f"🔍 Inspecting pool: {pool_path}")
        ...
        if layout == 'ticker-first':
            ...
            for file_path in parquet_files:
                ...
                try:
                    ...
                    logger.warning(f"   ⚠️  Error reading {ticker}/{timeframe}: {str(e)}")
            ...
            for file_path in parquet_files:
                ...
                try:
                    ...
                    logger.warning(f"   ⚠️  Error reading {ticker}: {str(e)}")
    ...
    if date_range == ("UNKNOWN", "UNKNOWN") and last_dates and first_dates:
        logger.info(f"   📅 Pool name has no date info, extracting from data...")
        date_range = extract_date_range_from_data(pool_path, first_dates, last_dates)
        logger.info(f"   📅 Detected date range: {date_range[0]} to {date_range[1]}")
...
def print_pool_summary(metadata: PoolMetadata):
    ...
    print("="*60)
    print(f"📁 Path: {metadata.pool_path}")
    print(f"📅 Date Range: {metadata.date_range[0]} to {metadata.date_range[1]}")
    print(f"🏷️  Tickers: {len(metadata.tickers)} ({', '.join(metadata.tickers[:5])}{'...' if len(metadata.tickers) > 5 else ''})")
    print(f"⏱️  Timeframes: {', '.join(metadata.timeframes)}")
    print(f"📊 Total Records: {metadata.total_records:,}")
    print(f"💾 Total Size: {metadata.total_size_mb:.1f} MB")
    print(f"🏥 Health: {metadata.health_status}")

    if metadata.issues:
        print(f"\n⚠️  Issues Found ({len(metadata.issues)}):")
    ...
    for ticker in sample_tickers:
        for tf in metadata.timeframes:
            ...
            if key in metadata.last_dates:
                ...
                print(f"   {ticker:12} {tf:10} → {last_date} ({records:>8,} records, {size:>6.1f} MB)")
