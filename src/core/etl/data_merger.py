...
def merge_parquet_files(
    ...
    logger.info(f"🔄 Merging {old_file.name}...")
    ...
    try:
        ...
        if new_data.empty:
            logger.warning(f"   ⚠️  New data is empty, nothing to merge")
            return False
        ...
        if has_overlap and strategy == 'append':
            logger.warning(
                f"   ⚠️  Overlap detected but strategy is 'append'. "
        ...
        if strategy == 'append' and not has_overlap:
            ...
            logger.info(f"   ✅ Merged successfully (fast path): {old_rows:,} + {new_rows:,} → {merged_rows:,} rows")
        else:
            ...
            logger.info(f"   ✅ Merged successfully: {old_rows:,} + {new_rows:,} → {merged_rows:,} rows")
...
def validate_merged_data(
    ...
    if high_violations.any():
        ...
        logger.warning(f"   ⚠️  Found {violation_count} rows where high price slightly below open/close (within tolerance)")
    ...
    if low_violations.any():
        ...
        logger.warning(f"   ⚠️  Found {violation_count} rows where low price slightly above open/close (within tolerance)")
        # Log sample violations for debugging
...
def merge_batch(
    ...
    logger.info(f"🔄 Batch merging {len(file_data_pairs)} files...")
    ...
    logger.info(f"\n✅ Batch merge complete: {successful}/{len(results)} successful, {failed} failed")
