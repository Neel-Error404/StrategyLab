...
def calculate_gaps(
    ...
    """
    logger.info(f"­ƒôè Calculating gaps...")
    ...
    if buffer_days > 0:
        ...
        logger.info(f"   Applied {buffer_days} day buffer ÔåÆ {target_date}")
    ...
    logger.info(f"   Ô£à Gap calculation complete")
...
def print_gap_report(report: GapReport):
    ...
    print(f"­ƒôà Gap Duration: {report.total_calendar_days} calendar days")
    print(f"­ƒôê Trading Days (est): {report.total_trading_days_estimate}")
    print(f"­ƒôè Records to Fetch (est): {report.total_records_estimate:,}")
    print(f"­ƒÆ¥ Data Size (est): {report.estimated_size_mb:.1f} MB")
    print(f"ÔÅ▒´©Å  Fetch Time (est): {report.fetch_time_estimate_min} minutes")
    print(f"Ô£à Status: {report.validation_status}")
    ...
    if report.validation_messages:
        print(f"\nÔØî Validation Issues:")
        for i, msg in enumerate(report.validation_messages, 1):
            print(f"   {i}. {msg}")

    # Show sample gaps
    print(f"\n­ƒôï Sample Gaps (first 3 tickers, all timeframes):")
    ...
    for (ticker, timeframe), (start, end) in [(k, report.gaps[k]) for k in sample_keys]:
        print(f"   {ticker:12} {timeframe:10} ÔåÆ {start.date()} to {end.date()}")
