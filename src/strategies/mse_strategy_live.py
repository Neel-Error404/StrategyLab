...
class MSEStrategy(StrategyInterface, UniversalStrategyTemplate):
    ...
    def calculate_macd_confidence(self, indicators: MSEIndicators, signal_type: str) -> float:
        """
        Sophisticated MACD-based confidence calculation
        
        Base Logic:
        - Start with 70% confidence when all 4 indicators align
        - Add strength bonuses based on MACD histogram magnitude
        - Apply trade-direction specific calculations
        """
        
        base_confidence = 70.0  # Reduced from 75% for more diversity
        
        # Extract histogram values
        hist_5min = indicators.macd_histogram_5min
        hist_15min = indicators.macd_histogram_15min
        
        if signal_type == "BUY":
            # BUY signals: Positive histograms indicate strength
            strength_5min = max(0, hist_5min) 
            strength_15min = max(0, hist_15min)
        else:  # SELL
            # SELL signals: Negative histograms indicate strength (use absolute value)
            strength_5min = max(0, -hist_5min)
            strength_15min = max(0, -hist_15min)
        
        # Normalize and distribute equally between timeframes
        # Each timeframe can contribute up to 12.5% (total 25% bonus)
        bonus_5min = min(strength_5min / 1.0, 1.0) * 12.5
        bonus_15min = min(strength_15min / 0.5, 1.0) * 12.5
        
        final_confidence = base_confidence + bonus_5min + bonus_15min
        
        # Bound between 50-95% for realistic range
        return min(max(final_confidence, 50), 95)
    ...
    def _check_entry_signal(self, symbol: str, indicators: MSEIndicators, current_price: float) -> Optional[Dict[str, Any]]:
        ...
        if (indicators.macd_line_5min > indicators.macd_signal_5min and      # 5min MACD bullish
            ...
            dynamic_confidence = self.calculate_macd_confidence(indicators, "BUY")
            ...
            return {
                ...
                "confidence": dynamic_confidence  # Dynamic MACD-based confidence
            }
        ...
        # SELL Entry: ALL 4 indicators must be bearish
            ...
            # Calculate dynamic confidence based on MACD histogram strength
            dynamic_confidence = self.calculate_macd_confidence(indicators, "SELL")
