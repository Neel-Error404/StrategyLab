...
- Live trading reference: keep `live module Contact details.md` (repo root) up to date; it documents how to read from `D:\Balcony\Trading\unified_trading_setup\trading_system_clean` without modifying it. Treat the live repo as read-only and only copy patterns into backtests when necessary.
...
- Before making changes in `analysis/portfolio_experiments/`, read `analysis/portfolio_experiments/MUST_READ.md`
  (workflow rules) and follow the referenced docs (`docs/RUN_MODES.md`, `docs/REDUNDANT_FILES.md`).
